    ...
    <section className="bg-white py-20 md:py-28 overflow-hidden">
      <div className="max-w-6xl mx-auto px-6 md:px-12">
        ...
        <div className="flex justify-center mb-4">
          <span className="bg-[#E8EEFE] text-[#1E4FBE] font-bold text-xs px-4 py-1.5 rounded-full uppercase tracking-widest">
        ...
        <div className="relative">
          ...
          <button
            onClick={() => scroll("left")}
            className="hidden md:flex absolute -left-6 top-1/2 -translate-y-1/2 z-10 w-12 h-12 bg-white shadow-lg border border-gray-100 rounded-full items-center justify-center hover:bg-[#E8EEFE] transition-colors"
          ...
          >
            {carouselSection.items.map((item, idx) => (
              ...
              >
                {/* Recipe image */}
                <div className="relative w-full aspect-[4/3] bg-[#F4F7FE]">
                  <Image
                    src={item.image}
                    alt={item.alt}
                    fill
                ...
                <div className="p-5">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {/* Time badge */}
                    <span className="inline-flex items-center gap-1 bg-[#E8EEFE] text-[#1E4FBE] font-bold text-xs px-3 py-1 rounded-full">
                      <Clock size={11} />
                      {item.time}
                    </span>
                    {/* Protein badge */}
          ...
          <button
            ...
            className="hidden md:flex absolute -right-6 top-1/2 -translate-y-1/2 z-10 w-12 h-12 bg-white shadow-lg border border-gray-100 rounded-full items-center justify-center hover:bg-[#E8EEFE] transition-colors"
        ...
        {/* Closing text */}
        <p className="text-center text-[#2F6FED] font-black text-xl mt-10 mb-8">
        ...
        <div className="flex justify-center gap-3 flex-wrap mb-10">
          {carouselSection.categories.map((cat, idx) => (
            <span
              key={idx}
              className="bg-[#E8EEFE] text-[#1E4FBE] font-bold text-sm px-4 py-2 rounded-full"
            >
              {cat}
            </span>
          ))}
        ...
        <div className="flex justify-center mt-12">
          <button
            ...
            className="animate-pulse-glow inline-flex items-center gap-2 bg-[#2F6FED] hover:bg-[#1E4FBE] text-white font-black text-base px-8 py-4 rounded-2xl transition-all duration-200 hover:scale-105 shadow-lg shadow-[#2F6FED]/25 uppercase tracking-wide"
          >
            {carouselSection.cta}
            <ArrowRight size={18} />
          </button>
